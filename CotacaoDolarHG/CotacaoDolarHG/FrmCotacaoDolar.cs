...
namespace CotacaoDolarHG
{
    public partial class FrmCotacaoDolar : Form
    {
        ...
        {
            string strURL = "https://api.hgbrasil.com/finance?array_limit=1&fields=only_results,USD&key=45c6e9dd";

            using (HttpClient client = new HttpClient())
            {
                try
                {
                    var response = client.GetAsync(strURL).Result; // ele atribui dinamicamente o tipo de variavel.
